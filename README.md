...
로그인 화면에서 심사 제출 폼으로 별도 전달한 계정을 사용하면 이메일 인증 없이
즉시 시연할 수 있습니다.

이 계정에는 현재 월 교대근무표, 최근 14일 상태·고민, 피부 프로필,
수면 시작·종료, HRV, 안정시 심박과 웨어러블 연결 상태가 목업으로 미리
들어 있습니다. 일반 회원가입·로그인은 기존 Supabase Auth를 그대로 사용하며,
심사용 계정은 운영 권한이나 관리자 페이지를 제공하지 않습니다. 계정 정보는
공개 저장소에 기록하지 않고 심사 제출 폼으로만 전달합니다.
