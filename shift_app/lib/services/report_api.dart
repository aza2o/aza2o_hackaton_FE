...
String formatSleepDurationMinutes(int minutes) {
  final safeMinutes = minutes < 0 ? 0 : minutes;
  final hours = safeMinutes ~/ 60;
  final remainder = safeMinutes % 60;
  if (hours == 0) return '$remainder분';
...
Future<AiReportResult> fetchAiReport({
  ...
  // 차트와 AI가 서로 다른 수면 부채를 말하지 않도록 단일 계산값을 쓴다.
  // 현재 actualMinutes는 워치 실측이 아니라 데모 데이터다.
  final sleepDebtMin = totalDeficitMin;
  final sleepDebtDisplay = formatSleepDurationMinutes(sleepDebtMin);
    ...
    res = await http.post(
      ...
      body: jsonEncode({
        ...
        'sleepSummary': {
          ...
          'totalDeficitDisplay': sleepDebtDisplay,
        },
        'skinRoutineContext': {
          'savedSkinProfile': {
          ...
          'recommendationRules': {
            'jawOrEarBreakout':
                '마스크·베개·이어폰·손 접촉과 세안 지연을 우선 점검하고 무거운 크림 중첩을 피한다',
  ...
  final generated = _briefComment(
    (json['comment'] as String? ?? '').replaceAll(
      '$sleepDebtMin분',
      sleepDebtDisplay,
    ),
  );
